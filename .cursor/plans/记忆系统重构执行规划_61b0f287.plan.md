...
todos:
  ...
  - id: update-service-skills
    content: 更新 style-fusion：路径更新（已移除“服务上下文”辅助 Skill）
    status: completed
  - id: update-commands
...
服务/模块上下文辅助 Skill：已移除（服务/模块上下文改为直接沉淀为 `memory/notes/` 的 `Kind=tech` 记忆笔记）
- 第 10、42 行：输出路径
...
- 服务/模块上下文：验证能正确写入 `memory/notes/`（Kind=tech）
