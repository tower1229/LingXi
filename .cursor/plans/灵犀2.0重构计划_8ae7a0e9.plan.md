...
todos:
  - id: create-req-command
    content: 创建 /req 命令（.cursor/commands/req.md），实现造物计划创建逻辑，包括任务编号生成、标题生成、需求提纯、需求放大、模板选择
    status: pending
  ...
  - id: update-auxiliary-skills
    content: 检查并更新辅助 Skills（plan-manager, index-manager 等），适配新文件命名和目录结构
    status: pending
  - id: cleanup-directories
    content: 删除旧目录结构（in-progress, completed），删除 INDEX.md，清理 session 目录
    status: pending
  - id: delete-old-files
    content: 删除 1.0 旧文件（flow-router, 旧阶段 Skills, flow.md 命令）
    status: pending
isProject: false
---
...
**功能**：创建造物计划（产出：`001.req.<标题>.md`）

**要点**：

- 自动生成任务编号（扫描 `.workflow/requirements/`，取最大编号+1）
- 自动生成标题（10字以内，特殊字符替换）
...
**统一目录**：`.workflow/requirements/`（无子目录）
...
- `.cursor/skills/req/`（需要检查是否与 req-stage 重复）
- `.cursor/skills/plan/`（需要检查是否与 plan-stage 重复）
- `.cursor/skills/work/`（需要检查，2.0 改名为 build）
- `.cursor/skills/review/`（需要检查是否与 review-stage 重复）
- `.cursor/skills/experience-*/`（保留并优化）
- 其他辅助 Skills（plan-manager, index-manager, service-loader 等）
...
**文件**：`.cursor/skills/plan-manager/SKILL.md`

**修改点**：

- 适配新文件命名格式（`001.plan.*.md` 而非 `REQ-xxx.plan.md`）
- 移除对 INDEX.md 的依赖（如存在）
...
1. 检查并更新 `plan-manager`、`index-manager` 等辅助 Skills
2. 更新其他依赖旧结构的 Skills

### 阶段 4：目录结构重构

1. 删除 `.workflow/requirements/in-progress/` 目录及内容
2. 删除 `.workflow/requirements/completed/` 目录及内容
3. 删除 `.workflow/requirements/INDEX.md`
4. 清理或删除 `.workflow/context/session/` 目录

### 阶段 5：删除旧文件

1. 删除 `.cursor/skills/flow-router/` 目录
2. 删除其他旧阶段 Skill 目录（如存在）
3. 删除 `.cursor/commands/flow.md`
...
- **规则**：扫描 `.workflow/requirements/` 目录，提取所有 `*.req.*.md` 文件的编号，取最大编号+1
- **起始值**：001
- **上限**：999
...
- **统一目录**：所有任务文档统一存放在 `.workflow/requirements/`
...
- [ ] 新文件统一存放在 `.workflow/requirements/`
- [ ] 无残留旧文件
