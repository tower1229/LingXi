...
- `.workflow/context/session/*.json`（1.0 会话状态文件，但保留 `pending-compounding-candidates.json` 的目录结构）

**保留但简化**：

- `.workflow/context/session/`（2.0 简化，仅保留 `pending-compounding-candidates.json` 用于经验候选暂存）
...
- `.cursor/commands/init.md`（需要适配 2.0，检查路径引用，特别是 `.workflow/context/session/` 相关路径）
- `.cursor/commands/remember.md`（需要适配 2.0，检查路径引用，特别是 `.workflow/context/session/pending-compounding-candidates.json`）
...
- 确认 `.workflow/context/session/pending-compounding-candidates.json` 路径仍然有效（2.0 中 session 目录简化但保留此文件）
...
4. 清理 `.workflow/context/session/` 目录（删除旧的会话状态文件，但保留目录结构用于 `pending-compounding-candidates.json`）
