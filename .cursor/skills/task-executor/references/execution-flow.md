# Task Executor — 完整执行流程

本文档为 task-executor 的详细步骤、使用场景与注意事项；SKILL.md 仅保留意图与关键约束。
...
1. 执行 `node .cursor/skills/task-executor/scripts/next-task-id.mjs` 获取下一个三位数编号；从项目根目录执行
...
- task 阶段定义"做什么"和"与什么集成"，详细实现细节留给 plan 和 build
...
- **其他**：采用「简单功能」模板（见 [task-doc-template-simple.md](task-doc-template-simple.md)）+ Agent 根据需求内容勾选必要章节（至少：概述、目标、功能需求/变更说明、验收清单）；不强制 UI/API/数据模型章节。
...
- 格式：`001.task.<标题>.md`（三位数编号 + 类型 + 标题，标题 10 字以内）

### 9. 下一步建议

**输出时机**：**只要本步写入了 task 文档**（即向 `.cursor/.lingxi/tasks/` 写入了 `*.task.*.md`），**必须在当轮回复末尾**输出下一步建议；与是否在回复中展示需求摘要无关。若本步未写入任何 task 文档（如仅做了澄清未写入），可不包含下一步建议。
...
**推荐逻辑**：自检通过（满足关键维度、无明显缺口）→ 若需求明显跨多模块、多文件或复杂度为中等/复杂，A 推荐 `/plan` 先拆解再 `/build`；简单任务可推荐直接 `/build`；B = `/vet` 查漏补缺，C/D 从允许集合选。自检不通过（存在明显缺失或模糊）→ A = `/vet` 或补充修改 task（推荐），B = `/plan`，C/D 从允许集合选。

**允许动作集合**（选项仅限下列）：`/vet <taskId>`、`/plan <taskId>`、`/build <taskId>`、补充/修改 task、其他/跳过。
...
本 Skill 由 `/task` 命令自动激活，执行逻辑完全由本 Skill 负责。Commands 只负责参数解析和产物说明。
