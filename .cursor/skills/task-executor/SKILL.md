...
2. **任务编号和标题生成**
   - 执行 `node .cursor/skills/task-executor/scripts/next-task-id.mjs` 获取三位数编号（在项目根目录执行）。
...
9. **下一步建议（有产物时必须输出）**
   ...
   - 选项仅允许：`/vet <taskId>`、`/plan <taskId>`、`/build <taskId>`、补充/修改 task、其他/跳过。
...
本 Skill 由 `/task` 自动激活；Command 仅负责参数承载与触发说明。
