...
**注意**：2.0 中不再输出质量准则建议。所有质量资产统一沉淀到 Experience 系统（团队级标准/经验或项目级经验）。
...
以下建议从近期沉淀中提炼，采纳后将沉淀到 Experience 系统（团队级标准/经验或项目级经验）。
**注意**：2.0 中不再输出质量准则建议。所有质量资产统一沉淀到 Experience 系统（团队级标准/经验或项目级经验）。
...
- **不得自动写入**：必须等用户明确选择存储目标（在 `/remember` 中选择团队级标准/经验或项目级经验）
- **明确标注为建议**：不是强制要求
