...
- **语义路径**：对 `.cursor/.lingxi/memory/notes/` 做语义搜索（概念级匹配）。构建语义查询时用自然语言说明“用户要解决的核心问题/决策点/约束”和“希望查找的记忆类型（原则/模式/风险/约束/排障路径等）”。取 **Top 0–5** 条候选。
- **关键词路径**：对同一目录及 INDEX 的 **Title、When to load** 列用 **Grep** 或工作区内容搜索，以 query 中的**技术词、错误码、配置项名、API 名、场景/维度词**等为查询做关键词/短语匹配。取 **Top 0–5** 条候选。
