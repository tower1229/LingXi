...
1. 提取风格特征（同 analyze）
2. 标准化风格向量
3. 读取 `.cursor/.lingxi/style-fusion/profile.json`
...
**执行步骤**：
1. 读取 `.cursor/.lingxi/style-fusion/profile.json`
2. 提取主导特征
3. 格式化输出
...
首次使用时，自动创建 `.cursor/.lingxi/style-fusion/` 目录并初始化：
...
3. **状态持久化**：所有状态存储在 `.cursor/.lingxi/style-fusion/`
