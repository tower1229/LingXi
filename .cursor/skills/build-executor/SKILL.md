...
测试即验收目标。对每个可单元测试的单元，按以下四步执行，不能通过改测试来通过验收：
...
使用 TDD 时不为尚不存在的功能编写 mock 实现（与 Cursor 官方 TDD 工作流一致）。若 plan 中已按「Txa 编写测试 → Txb 实现」排列，则 Txa 对应步骤 1–2、Txb 对应步骤 3–4；未细分时按模块/文件划分单元并套用上述循环。

#### 2.3 按单元循环：代码实现与测试（Plan-driven）

按 plan 任务清单顺序，对每个可单元测试单元执行「先测再实现」循环：

- **步骤 A**：基于 `<taskId>.testcase.*.md` 中该单元的行为与规格，**仅**编写该单元的测试脚本（不实现该单元）。
- **步骤 B**：运行该单元测试，确认失败或基线（未实现时应为失败）。
- **步骤 C**：实现该单元（按 plan 任务描述与 req），不修改已写测试。
- **步骤 D**：运行该单元测试，失败则只改实现、循环直到通过；通过后再进入下一单元。

若 plan 中任务已明确为「Txa: 编写测试」「Txb: 实现」，则 Txa 对应 A、Txb 对应 C+D；若 plan 未细分，则按模块/文件划分单元并套用 A→B→C→D。

**实现时**：按 req 的技术方案、plan 的依赖与顺序；若子任务 ≥3 或存在明显依赖，使用结构化任务列表并标注依赖。**先探索再修改**：对不熟悉模块或目录，先语义搜索或读取关键文件建立理解，再实现或修改代码。
...
#### 3.5 编写测试脚本与先测再实现（基于 req 验收标准）

对每个可测单元采用「先写该单元测试 → 跑确认红 → 实现 → 跑直到绿」：

- 基于 req 的验收标准（及若有 testcase 则引用）**先仅**为该单元编写测试，不实现该单元逻辑
- 运行该单元测试，确认失败或基线
- 实现该单元，不修改已写测试；运行测试，只改实现直到通过
- 通过后再进入下一单元

覆盖 req 中定义的功能验收清单；遵循检测到的测试框架。不发明未在 req/testcase 中定义的行为。

#### 3.6 执行单元测试（循环修复直到全部通过）

- 每个单元按 3.5 先写测试、确认红、再实现、再跑直到通过
- 记录测试执行结果（通过/失败）
- **如果测试失败**：
  - 分析失败原因，定位 bug
  - 只修复实现（不修改测试）
  - 重新执行单元测试
  - **循环执行**：修复 → 测试 → 修复 → 测试，直到该单元所有测试全部通过
- **只有当前单元所有测试通过后，才继续下一个模块或任务**
...
3. 按 plan 任务清单顺序，对每个可测单元执行先测再实现（编写该单元测试 → 运行确认红/基线 → 实现该单元 → 运行直到绿）；支持按单元先测再实现（TDD）
4. 文档同步（基于 plan 清单）
5. 快速 E2E 冒烟测试（可选，基于语义判断）
...
3. Agent 自行拆解任务
4. 测试框架检测与安装（如需要）
5. 对每个可测单元执行先测再实现：编写该单元测试（基于 req/若有 testcase）→ 运行确认红 → 实现该单元 → 运行直到绿
6. 文档同步（Agent 识别）
