...
**输出时机**：仅当本次回复已有正文输出（如实现说明、测试结果说明等）时，在末尾追加下一步建议；纯静默成功（仅实现与测试通过、无其他输出）时不追加。

**质量自检维度**（给出建议前先自评当前产出）：是否已按 req/plan 实现？约定测试是否通过？是否有明显未完成项？

**推荐逻辑**：自检通过 → A = `/review` 做交付审查（推荐），B = `/remember` 沉淀，C = 先改代码再 review，D = 其他。自检不通过 → A = 先改代码再 review（推荐），B = `/review`，C/D = 其他。
