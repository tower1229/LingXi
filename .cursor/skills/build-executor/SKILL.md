...
- **Req-driven**：仅 req，无 plan/testcase
...
（共用）测试即验收目标。对每个可单元测试的单元，按以下四步执行，不能通过改测试来通过验收：
...
- **Req-driven**：测试规格与验收来自 req（及若有 testcase 则引用）；不发明未在 req/testcase 中定义的行为。

#### 2.4 按单元循环：代码实现与测试

（共用）对每个可单元测试单元执行「先测再实现」循环：

- **步骤 A**：基于该单元的行为与规格**仅**编写该单元的测试脚本（不实现该单元）。**Plan-driven**：规格来自 `<taskId>.testcase.*.md`。**Req-driven**：规格来自 req 验收标准（及若有 testcase）。
