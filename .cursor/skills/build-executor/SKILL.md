...
- **Req-driven**：若存在 `<taskId>.testcase.*.md` 则一并读取；**若不存在 testcase**，调用 **testcase-designer** skill（输入：当前 req 文档），生成完整 testcase 并写入 `<taskId>.testcase.<标题>.md`，再进入 2.2。
...
3. **模式选择**：根据是否存在 plan 文件自动选择 Plan-driven 或 Req-driven 模式。
4. **文档同步**：确保代码与文档保持一致
