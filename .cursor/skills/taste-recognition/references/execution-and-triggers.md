...
本文档为 taste-recognition 的触发点与输入约定、执行步骤、与环节品味嗅探的关系；SKILL.md 保留意图与扩展 payload 契约（含 layer、可选 l0OneLiner/l1OneLiner、patternHint；门控）。
...
无可沉淀时静默；有可沉淀时经升维后产出扩展 payload；**判定不写**的条目不产出、不加入 payloads。主 Agent **仅当 payloads 非空时**将 payload（单条或多条）组成 **payloads 数组**传入 lingxi-memory。

## 本 Skill 的执行步骤

1. 确定触发场景（/remember / /extract / /init 写入 / 环节选择题）→ 确定输入范围与 source。
...
5. **若有可沉淀**：从输入中抽取 scene、principles、choice 及可选的 evidence；**先做模式靠拢**（参考 references/pattern-catalog.md），若匹配则更新 principles/choice 或设置 patternHint + patternConfidence。
6. **升维判定**：对（可能已模式升维的）内容按 references/elevation-rules.md 做**四维评分**，计算 T 与 D1–D4；若 T≤3 或触犯例外不写 → **不产出该条**，不加入 payloads。
7. 对建议写入的条目标注 layer、可选 l0OneLiner/l1OneLiner；产出符合**扩展后** payload 规范的 JSON；同一轮多条则组成 payloads 数组。
8. 主 Agent **仅当 payloads 非空时**调用 lingxi-memory；禁止将原始用户消息或对话片段作为 lingxi-memory 的输入。
