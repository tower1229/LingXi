...
1. **识别（含前置排除）**：从当前触发点的输入中判断是否可沉淀（偏好、约束、取舍、决策等），并前置排除 Exclusions：secrets/tokens/credentials/private personal data、one-off task instructions、transient details（如分支名、commit hash、临时错误）。无可沉淀或命中排除项则静默返回，不产出 payload、不调用 lingxi-memory。
