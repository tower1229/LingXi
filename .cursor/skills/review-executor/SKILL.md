...
**输出时机**：**只要本步完成了审查**（已执行审查逻辑并得出结论，无论是否写入 review 报告文件），**必须在当轮回复末尾**输出下一步建议；与是否在回复中展示审查报告全文无关。不可省略下一步建议。
...
**推荐逻辑**：审查通过 → A = 本任务已交付 / 无后续（推荐），B = `/remember` 沉淀，C = 再次执行 /review，D = 其他。有 Blockers/High → A = 修复后 `/review`（推荐），B = `/remember` 记问题，C = 暂不修，D = 其他。
