...
**可验证目标前置（推荐）**：若项目存在测试、类型检查或 lint，审查前优先执行或说明其执行状态，再进入各维度审查；以便审查结论基于可验证信号。
...
**输出时机**：**只要本步完成了审查**（已执行审查逻辑并得出结论，无论是否写入 review 报告文件），**必须在当轮回复末尾**输出下一步建议；与是否在回复中贴出审查报告全文无关。不可省略下一步建议。

**必须包含的结构**（在回复末尾）：标题行「**下一步可尝试（选一项）**」+ 四项「**A) …（推荐）**」「**B) …**」「**C) …**」「**D) …**」；给出建议前结合审查结论做自检（见下），根据自检结果决定 A 的推荐内容。

**质量自检维度**（给出建议前结合审查结论）：审查结论是否为「通过」？是否仍有 Blockers/High 未关闭？

**推荐逻辑**：审查通过 → A = 本任务已交付 / 无后续（推荐），B = `/remember` 沉淀，C = 再跑一遍 review，D = 其他。有 Blockers/High → A = 修复后 `/review`（推荐），B = `/remember` 记问题，C = 暂不修，D = 其他。

**允许动作集合**（选项仅限下列）：修复后再 `/review <taskId>`、本任务已交付/无后续、运行测试/类型检查后再 review、`/remember …`、暂不修/其他。（灵犀无「关闭任务」系统动作，审查通过时用「本任务已交付/无后续」表示无需后续操作。）
