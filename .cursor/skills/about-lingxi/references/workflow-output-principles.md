...
本文档是灵犀工作流命令（/init、/req、/plan、/build、/review、/review-req、/remember）及对应 executor Skill 的**输出与静默契约**。由各 Command 与 Skill 文档引用本文件统一约束，不通过全局 Rules 注入。
...
req/plan/build/review/review-req 在产生产物（写入文档或输出审查结果）后，**必须在当轮回复末尾**输出「下一步可尝试（选一项）」及 A/B/C/D 四项，与是否贴出产物摘要无关；保证用户每步结束后都能看到可选的下一动作。格式与允许集合见各 executor Skill 的「下一步建议」小节。
...
**场景**：写入 req 文档成功；仅澄清未落盘；写入失败。

**正例**：

- 写入成功且本步有正文输出时：回复末尾仅带下一步建议块，不贴「已写入 `001.req.xxx.md`」等确认。
- 仅澄清未落盘：可不输出或一句「需求已澄清，需要我落盘为 req 文档吗？」；未写入则无需下一步建议块。
- 写入失败：只输出错误原因与可操作建议（如路径不可写、权限不足），不输出过程旁白。

**反例**：

- 「开始收集需求…」「正在根据 req-executor 执行…」—— 过程旁白，违反静默。
- 「已成功写入 req 文档，路径为 `.cursor/.lingxi/tasks/001.req.xxx.md`。」—— 冗余确认，成功时应静默或仅通过下一步建议引导。
...
**约定**：若实现已偏离 req/plan，下一步可尝试中可包含「撤销更改 → 修订 plan 或 req → 重新 /build」。

**正例**：

- 通过时：可不贴实现/测试详情，末尾必须带下一步建议块。
...
**约定**：审查前若项目存在测试、类型检查或 lint，可优先执行或说明其状态，再进入各维度审查；下一步建议中可包含「运行测试/类型检查」等可验证动作选项。若实现已偏离 req/plan，下一步可尝试中可包含「撤销更改 → 修订 plan 或 req → 重新 /build」。
...
- 无清单或静默完成时：可一句引导（如「用 /req、/build 等开始任务」）或完全静默。
