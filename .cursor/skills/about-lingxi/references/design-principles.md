...
- 文件写入静默：写入成功时可不输出文件内容摘要，失败时输出错误信息
- 工作流步骤有产物时：req/plan/build/review/review-req 在产生产物（写入文档或输出审查结果）后，**必须在当轮回复末尾**输出「下一步可尝试（选一项）」及 A/B/C/D 四项，与是否贴出产物摘要无关；保证用户每步结束后都能看到可选的下一动作
