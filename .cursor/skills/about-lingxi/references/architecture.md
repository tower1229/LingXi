...
**定位**：系统的决策中枢。所有用户请求统一经由调度层驱动执行，调度层自身不直接执行业务代码或重度 I/O，从而保持**单一职责**与**可审计性**。

**核心机理**：

- **脚本与 Agent 分工 (Script-Agent Separation)**：调度层分为两个执行主体——`beforeSubmitPrompt` **脚本**（Phase 0）负责所有确定性的纯文件操作（HOT_RAM 初始化、心跳调度、用户配置注入），在 Agent 启动前同步完成；**主 Agent**（Phase 1-3）专注需要 LLM 语义理解的操作（记忆检索、品味识别、Megaprompt 组装、Subagent 派发与后处理）。职责边界清晰，Agent 不兜底脚本层的工作。
...
| 步骤 | 职责 | 主要产物 / 输出 |
|------|------|-----------------|
| **task** | 需求提纯与放大，产出可验收的 task 文档 | `.lingxi/tasks/<taskId>.task.<标题>.md` |
...
| 层级 | 在工作流中的角色 |
|------|------------------|
| **调度层** | 判定请求属于哪一工作流步骤；统一走四阶段管道（Phase 0-3）；工作流各步骤在 Phase 1 预处理后由 Phase 2 派发 Subagent 执行；编排、下一步建议、状态读写均经调度层。 |
