...
- **记忆沉淀**：由用户通过 `/remember`、`/init` 或 `/refine-memory` 触发；经 taste-recognition 产出 payload 后，以 **payloads 数组**交由 **lingxi-memory** Subagent 在独立上下文中执行（校验→映射→治理→门控与直接文件写入 notes + INDEX，主对话收简报）。
...
- **sessionStart**（`session-init.mjs`）：在会话开始时**仅**注入记忆检索约定（每轮先执行 `/memory-retrieve <当前用户消息>`）及 conversation_id 传入约定；**不注入**记忆沉淀约定。沉淀由用户通过 /remember、/refine-memory、/init 触发；其他审计/门控为可选。
...
- **lingxi-memory**（`.cursor/agents/lingxi-memory.md`）：承担**记忆写入**。通过**显式调用**使用；**仅接受** taste-recognition skill 产出的 **payloads 数组**（7 字段品味：scene, principles, choice, evidence, source, confidence, apply）（禁止传入原始对话或旧形态 input）。用户通过 `/remember`、`/init` 或 `/refine-memory` 触发；主 Agent 先调用 taste-recognition 产出 payload（单条或批量），组成 payloads 数组后调用 lingxi-memory。子代理在独立上下文中完成校验→映射→治理→门控与**直接文件写入**（notes + INDEX），主对话收简报。
