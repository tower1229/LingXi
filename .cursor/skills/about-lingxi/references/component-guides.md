...
- **lingxi-session-distill**（`.cursor/agents/lingxi-session-distill.md`）：**会话提炼**。由 30min 心跳触发时主 Agent 在步骤 A 调用，传入 candidate_ids 与 enqueued_by；在后台对入队会话做 taste-recognition 提炼并调用 lingxi-memory 写入，主会话不等待。
