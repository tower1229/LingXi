...
**记忆系统**（四部分：自动沉淀、手动记忆、记忆写入、记忆提取；前三部分共同组成**记忆沉淀**）：

- **记忆沉淀**：**自动沉淀**（session 约定触发，主 Agent 判断可沉淀后调 lingxi-memory）、**手动记忆**（用户 `/remember` 或 `/init` 主动发起）、**记忆写入**（由 **lingxi-memory** Subagent 在独立上下文中执行；仅接受 taste-recognition skill 产出的 7 字段品味 payload，校验→映射→治理→门控与直接文件写入 notes + INDEX，主对话仅收一句结果）。
- **记忆提取**：`memory-retrieve`（Skill）——每轮回答前检索并最小注入（由 sessionStart hook 注入的约定触发）。

**Hooks（记忆注入约定）**：

- sessionStart（`.cursor/hooks/session-init.mjs`）：在会话开始时注入「每轮先执行 /memory-retrieve <当前用户消息>」的约定**以及【记忆沉淀约定】**（先调用 taste-recognition skill，有 payload 再调 lingxi-memory），保证**记忆沉淀**（自动沉淀与手动记忆 /remember、/init）在每会话内生效，安装后即生效；无匹配静默，失败可降级
...
- **sessionStart**（`session-init.mjs`）：在会话开始时注入记忆检索约定（每轮先执行 `/memory-retrieve <当前用户消息>`）**以及【记忆沉淀约定】**（先调用 taste-recognition skill，有 payload 再调 lingxi-memory），**记忆沉淀**（自动沉淀与手动记忆 /remember、/init）均可用，安装后即生效；其他审计/门控为可选。
...
- **lingxi-memory**（`.cursor/agents/lingxi-memory.md`）：承担**记忆写入**。通过**显式调用**使用；**仅接受** taste-recognition skill 产出的 7 字段品味 payload（scene, principles, choice, evidence, source, confidence, apply）（禁止传入原始对话或旧形态 input）。可在提示中使用 `/lingxi-memory` 语法或自然语言提及「lingxi-memory 子代理」并传入 payload。**自动沉淀**依赖 sessionStart 注入的【记忆沉淀约定】（先调用 taste-recognition skill，有 payload 再调 lingxi-memory）；**手动记忆**（用户 `/remember` 或 `/init` 写入）随时可用。安装后两种沉淀方式均生效；子代理在独立上下文中完成校验→映射→治理→门控与**直接文件写入**（notes + INDEX），主对话仅收一句结果。
