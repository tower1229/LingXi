...
- sessionStart（`.cursor/hooks/session-init.mjs`）：在会话开始时注入「每轮先执行 /memory-retrieve <当前用户消息>」的约定**以及【记忆沉淀约定】**（先调用 taste-recognition skill，有 payload 再调 lingxi-memory），保证自动沉淀在每会话内生效，安装后即生效；无匹配静默，失败可降级
...
- **sessionStart**（`session-init.mjs`）：在会话开始时注入记忆检索约定（每轮先执行 `/memory-retrieve <当前用户消息>`）**以及【记忆沉淀约定】**（先调用 taste-recognition skill，有 payload 再调 lingxi-memory），自动沉淀与 /remember、/init 主动沉淀均可用，安装后即生效；其他审计/门控为可选。
...
- **lingxi-memory**（`.cursor/agents/lingxi-memory.md`）：记忆写入。通过**显式调用**使用；**仅接受** taste-recognition skill 产出的 7 字段品味 payload（scene, principles, choice, evidence, source, confidence, apply）（禁止传入原始对话或旧形态 input）。可在提示中使用 `/lingxi-memory` 语法或自然语言提及「lingxi-memory 子代理」并传入 payload。**自动沉淀**依赖 sessionStart 注入的【记忆沉淀约定】（先调用 taste-recognition skill，有 payload 再调 lingxi-memory）；**主动沉淀**（用户 `/remember` 或 `/init` 写入）随时可用。安装后两种沉淀方式均生效；子代理在独立上下文中完成校验→映射→治理→门控与**直接文件写入**（notes + INDEX），主对话仅收一句结果。
