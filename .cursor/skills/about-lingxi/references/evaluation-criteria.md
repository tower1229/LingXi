...
- [ ] **是否有助于经验沉淀？**（有助于记住创造者的判断力、品味和责任感）
- [ ] **是否有助于经验提取？**
- [ ] **是否有助于持久化记忆能力？**

### AI Native

**方向性原则**：与 core-values 中 AI Native 一致——尊重 AI 能力、预留进化空间，关键决策以人为主、门控保障。以下检查项据此展开（如是否信任模型、是否采用短说明、门控是否服务于人等），不限于列举项。

- [ ] **是否信任模型？**（以模型为能力源泉，少干涉，用少量工具与简短提示即可执行）
- [ ] **是否为 AI 进化预留空间？**（不固化能力边界，尊重智能匹配能力）
- [ ] **是否采用短说明风格？**（在有效传达设计意图的前提下，核心文件是否简短、少冗余）
- [ ] **门控是否服务于人？**（关键决策由人拍板，门控尺度由人决定；高风险动作是否有门控且不替代推理）
- [ ] **是否把 AI 用在高语义/高歧义/需权衡的环节？**
- [ ] **是否避免过度约束 AI？**（依赖自然语言理解与上下文判断，而非硬编码规则、关键词列表或复杂 if-else）
...
- 门控与必要契约是否用于关键决策与高风险动作，且尺度由人决定？
- 是否控制上下文体积与调用链路，避免 token 成本失控？
...
- [ ] **是否需要同步更新 `about-lingxi` 中的相关文档？**
  - 如果改动涉及 Commands、Skills、Rules、Hooks、Subagents 的增减或职责变更 → 更新 `references/architecture-blueprint.md` 和 `references/component-guides.md`
