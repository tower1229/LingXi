...
**功能覆盖**：快速了解灵犀、workflow 调优与价值判定、组件选择、架构与工程实践评估。

**Quick Start**：按场景加载 references（核心价值、架构、机制、设计原则、评价准则、组件指南）；进行价值对齐/架构合理性/工程实践评估；输出摘要或改进建议。
...
- 记忆写入由 **Subagent lingxi-memory** 在独立上下文中执行（双入口、直接文件写入）；**自动沉淀**（mode=auto）与**主动沉淀**（/remember）均由 sessionStart 注入的【记忆沉淀约定】保证，安装插件即生效
...
- 架构合理性检查（组件选择、职责分离）
- AI Native 平衡判据（4 维判断器、项目版组件选型决策表、门控与成本检查）
