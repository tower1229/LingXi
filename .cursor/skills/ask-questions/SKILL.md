...
统一「怎么问、怎么收敛结果」的协议，避免各处重复定义与语义漂移；不负责业务决策。适用于单选/多选交互及统一异常处理（无选择、无效选择、取消、重试）。

## 交互契约（统一）

1. 始终使用 ask-questions 交互收集选择。
2. 输入统一使用 `questions[]`，每题 `question_id` 使用语义化 id（如 `next_step`、`write_strategy`）。
...
当 ask-questions UI 无法唤起时，使用以下固定排版：
