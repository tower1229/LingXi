...
### 5.5) Trade-off Record 输出（可选）

在实现过程中的关键取舍处，若出现关键取舍、风险接受、拒绝方案等价值取向判断，应输出 Trade-off Record。参考 `references/trade-off-record.md`。

**输出时机**：
- 在实现过程中发现 root cause 并更换方案
- 明确接受某个实现风险（而非规避）
- 拒绝某个实现方案，且拒绝理由值得记录
...
在阶段切换前，必须检查可推进判据。参考 `references/gate-protocol.md` 中的 `work → review` 检查清单：
