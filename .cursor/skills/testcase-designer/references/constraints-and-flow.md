...
本 Skill 由 plan-executor（主产出）、build-executor（Req-driven 且无 testcase 时）、review-executor（覆盖审计）显式调用。从 req 文档产出结构化 testcase 文档，保证 F→TC 映射与验证方式一致。
...
1. **解析 req**：读取「4. 功能需求」表及验收标准、成功标准、验收检查清单；提取每条 F 的需求编号、需求描述、验收标准、验证方式（unit/integration/e2e/manual/rubric）、边界/异常、证据形式、优先级。
...
plan-executor 调用本 Skill 产出 testcase；build-executor 在 Req-driven 且无 testcase 时调用并生成后再继续；review-executor 按相同 F→TC 映射与验证方式做覆盖审计。

## 注意事项

不发明 req 中未出现的需求或验收标准；边界/异常从 req 推导，不随意扩展；模板以本 Skill 的 references 为 SSoT。
