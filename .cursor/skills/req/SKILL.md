...
- 满足 → **必须执行步骤 9（REQ 文档候选提取）**，然后静默推进
- 不满足 → 输出检查清单（仅在异常时展示）

### 9) REQ 文档候选提取（判据满足后、进入 plan 前执行）

在进入 plan 前，从 REQ 文档提取高信号片段并输出候选注释。**重点关注任务驱动力背后的价值判断**。

**提取来源（按价值密度排序）**：

1. **背景（1.1 背景）**：为什么需要这个功能？背后的业务驱动、痛点、上下文。包含"什么被认为是重要的/有问题的"等价值判断。
2. **问题描述（1.2 问题描述）**：如何识别和描述问题？什么被认为是问题（价值判断）？问题的优先级和影响。
3. **解决方案概述（1.3 解决方案概述）**：为什么选择这个方案？背后的设计原则和价值判断（如"遵循静默成功原则"、"提升可读性"等）。约束条件的选择理由。
4. **成功标准（2.3 成功标准）**：为什么这样定义成功？背后的价值取向（如"可读性"、"直观性"、"可维护性"等）。
5. **关键决策记录（6.2 关键决策记录）**：如果有，这是明确的取舍记录，包含备选方案、拒绝理由、接受的风险。
6. **非目标（2.2 非目标）**：**仅当非目标包含明确的"为什么不做"的理由时**，才提取。单纯列出"不做X"而没有理由的，不提取。

**提取原则**：

- **聚焦价值判断**：提取"为什么"而非"是什么"。关注背后的价值取向、设计原则、取舍理由。
- **识别设计原则**：如果解决方案中体现了明确的设计原则（如"静默成功"、"人类友好"、"Fail Fast"等），应提取。
- **识别问题识别模式**：如果背景或问题描述中体现了"如何识别问题"的模式，应提取。
- **识别约束选择理由**：如果解决方案中选择了某些约束条件，应提取选择理由。
...
<!-- EXP-CANDIDATE {
  "stage": "req",
  "sourceStage": "req",
  "trigger": "当遇到类似需求背景/问题/方案选择时",
  "decision": "从 REQ 文档提取的价值判断/取舍决策/设计原则",
  "alternatives": ["被拒绝的方案/选项（如有）"],
  "signal": "判断依据/价值取向/设计原则",
  "solution": "最终方案/原则/判断",
  "verify": "后续如何验证这一判断",
  "pointers": ["REQ-xxx.md#1.1", "REQ-xxx.md#1.3"]
} -->
