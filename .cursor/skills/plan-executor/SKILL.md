...
产出 plan + testcase 文档：将 req 拆解为可执行任务（含 F→T 映射、先测再实现顺序、文件变更清单），并产出与 req 验证方式对齐的 testcase。能力：读 req/代码、语义搜索、Web/context7 深度调研、必要时 ask-questions 澄清。
...
- **下一步建议**：只要写入了 plan 或 testcase，必须在当轮回复末尾输出「**下一步可尝试（选一项）**」+ 四项 A/B/C/D；允许集合：`/build <taskId>`、调整 plan、回头看 req、其他/跳过。

## 产物与 References

- **产物**：`001.plan.<标题>.md`、`001.testcase.<标题>.md`（标题 10 字以内，从 req 提取）。
