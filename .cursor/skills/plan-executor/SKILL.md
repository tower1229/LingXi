...
当需求拆解为多个子任务且存在依赖时，使用结构化任务列表并标明依赖关系。基于 req「4. 功能需求」表格；可结合**步骤 3** 代码库分析、**步骤 4.2** 外部知识结论。读取功能需求表格，分析需求描述/实现方案/验收标准/验证方式/优先级，拆解为可执行任务（T1, T2, ...），**强制生成 F→T 映射**：每个任务在 plan 任务清单中标注**关联需求**（对应的 F 编号，如 F1、F1,F2）。识别依赖关系，从实现方案与代码库分析结果提取文件路径。本步产出的任务清单作为**步骤 6 任务拆解**的输入。
...
基于**步骤 5.2** 的测试用例基础与 **req 中每条 F 的验证方式**（unit/integration/e2e/manual/rubric）做行为提取与规格化，供 build 先测再实现使用。

**与验证方式对齐**：单元测试对应 req 验证方式 `unit`；集成测试对应 `integration`；E2E 对应 `e2e`；对验证方式为 `manual` 或 `rubric` 的需求，产出可执行步骤、预期结果，以及 rubric 时的评分维度与记录模板（见 testcase 模板「5. 手工验证与评审」）。
...
- [ ] 覆盖 req 中的所有功能需求
- [ ] **每条 F（验证方式不为空）均有 TC 或手工/rubric 区块覆盖**
- [ ] 每个验收标准有对应的验证方式
- [ ] 任务可独立执行，依赖关系明确，任务清单含 F→T 映射（关联需求列）
- [ ] 文件变更清单完整（包含测试文件）
- [ ] 测试任务作为独立分类存在
- [ ] 可测试行为已从需求中提取，测试设计与 req 验证方式对齐
- [ ] 单元测试规格包含输入/输出/边界条件
- [ ] 复杂任务有规范引用
