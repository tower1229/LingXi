...
- **目标**：产出一份符合模板的 req 文档。
- 项目上下文分析 → 任务编号/标题生成 → 需求提纯 → 类型与复杂度评估 → 需求放大 → 按需求类型选模板 → 信息汇总 → 生成文档（见 references）→ 输出下一步建议。
...
- 将识别到的隐含意图以建议形式呈现，用 `questions` 交互确认（options 仅用 label，如「确认采纳」「部分采纳」「拒绝」）
- 避免过度推断，只提出高置信度的隐含需求
...
- 输出需求理解总结
- 用 `questions` 交互确认（options 仅用 label，如「确认」「需修改」「取消」）
...
- **其他**：采用「简单功能」模板（见 [references/req-doc-template-simple.md](references/req-doc-template-simple.md)）+ Agent 根据需求内容勾选必要章节（至少：概述、目标、功能需求/变更说明、验收清单）；不强制 UI/API/数据模型章节。

### 7. 信息汇总（含 Fail Fast）

在写入前，将收集的信息汇总为可执行决策。**先执行 Fail Fast 检查**，必要信息齐全后再汇总。

- **Fail Fast**：若必要信息缺失（功能目标、目标用户（前端需求）、核心场景（前端需求）），一次性列出并给出 2-3 个选项，使用 `questions` 交互（遵循 [questions-interaction](.cursor/skills/questions-interaction/SKILL.md) 契约）等待用户选择后再写入；options 仅用 label 展示选项（如「选项1」「选项2」），含取消选项；无有效选择时重试当前问题。重要信息（成功标准、优先级、特殊约束）模糊时建议澄清。
...
- **简单功能**或**其他**：使用 [references/req-doc-template-simple.md](references/req-doc-template-simple.md)；主类型为「其他」时按需选章节。
...
7. 模板选择：完整模板
8. 信息汇总（含 Fail Fast 检查、记忆融入：每轮注入的约定 + memory-retrieve）
9. 文档生成
10. 下一步建议
...
3. **Fail Fast**：信息汇总时若必要信息缺失必须询问，不猜测；各前置步骤发现缺失时应立即提问
