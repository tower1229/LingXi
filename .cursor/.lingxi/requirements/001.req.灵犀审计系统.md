...
- 记忆库写入发生在 lingxi-memory 子代理内部，主 Agent 的 Hooks 看不到具体对哪条 note 做了增删改，需要记忆侧主动打点以实现事件级审计与会话级关联。
...
| F5   | lingxi-memory 子代理从 input 解析 conversation_id、generation_id；每次写 note/INDEX 后追加一条记忆审计行到同一 audit 文件                                                                         | `.cursor/agents/lingxi-memory.md` 约定解析与写入；写入逻辑在子代理执行写盘后追加 NDJSON 行                                         | 写记忆后 audit 中出现 memory*note*\* 行且含 conversation_id | 必须实现 |
| F6   | 记忆审计事件类型：memory_note_created、memory_note_updated、memory_note_deleted、memory_index_updated（可选）；字段含 ts、event、conversation_id、generation_id、note_id、operation、source、file | 子代理写盘后根据操作类型生成对应 event 与字段，appendFileSync 同一 audit 文件                                                      | 创建/更新/删除 note 后对应 event 与字段正确                 | 必须实现 |
...
- 记忆审计由 lingxi-memory 子代理在现有「产候选 → 治理 → 门控 → 写盘」流程末尾追加一次 append，不改变现有写入语义。
...
| 风险                          | 影响                   | 缓解措施                                                         |
| ----------------------------- | ---------------------- | ---------------------------------------------------------------- |
| Hook 脚本超时或异常           | 主流程被阻塞或误拦     | 脚本内快速返回、catch 后统一返回放行 JSON；Hook 配置可设 timeout |
