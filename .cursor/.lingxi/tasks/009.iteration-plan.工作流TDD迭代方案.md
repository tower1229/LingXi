...
`/task` 负责产出“可实施”的任务文档，至少包含：

1. 功能目标与边界（含非目标）。
2. 技术方向与可行性结论（架构层级）。
3. 每条需求的验收标准与验证方式。

注意：不要求在 task 阶段写测试代码，但必须写可测试规格。

### 3.2 Vet（设计质量共审）

`/vet` 负责审查 task 文档质量，不写实现细节。重点：
...
1. 按 task/plan/testcase 实施，不重定义需求。
2. 对可测单元执行“先测再实现”循环。
...
2. `build-executor`：读取 testcase；缺失时调用 `testcase-designer` 生成testcase。。
...
### 6.1 task-executor 与模板

更新：

1. `.cursor/skills/task-executor/SKILL.md`
2. `.cursor/skills/task-executor/references/task-doc-template-full.md`
3. `.cursor/skills/task-executor/references/task-doc-template-simple.md`
...
1. `.cursor/skills/vet-executor/SKILL.md`
