...
心跳相关事件（会话提炼）：`heartbeat.triggered`（主 Agent 写入：ts、conversation_id、candidate_ids、reason）；`heartbeat.distillation_completed`（lingxi-session-distill 子代理写入：ts、candidate_ids、sessions_processed、payloads_written、conversation_id 可选）；`heartbeat.distillation_failed`（子代理写入：ts、candidate_ids、error_preview）。
