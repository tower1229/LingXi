...
| payload → note 映射 | 从 payload 生成 note 各字段的规则（含 Title、Scope、TasteKey、Supersedes） | §6.5 |
| 记忆键设计 | taste_key 结构、存储、与「不再问」及 memory-retrieve 的配合 | §7.2 |
...
| **3. /init command** | init-executor 在用户确认写入记忆时，委托主 Agent 或直接调用 | /init 流程中用户确认后的草稿（类型化收集结果、目标/非目标、用户与流程、风险、发布方式等）；可按「一条记忆一个 payload」拆成多条 | `init` | 用户已确认的草稿建议 high；未完全确认的项可 medium |
...
品味识别 skills：判断本轮是否存在可沉淀偏好
    ...
    └── 有 ──► 产出品味 payload（§6.4 规范）
             ...
             lingxi-memory：校验 → 由 payload 映射生成 note 字段（缺则补全）→ 治理（近邻 TopK）
                    ...
                    ▼
             门控（有条件必选）：
               · merge / replace → 必须 questions 门控，不得静默
               · new → 高可靠性可静默写入，低可靠性必须 questions 门控
                    │
                    ▼
...
| Kind | 规则 | payload.source=init 且为类型化草稿 → `decision` 或 init-checklists 约定；choice 为原则名且 scene 泛化 → `principle`；其余默认 `decision`，按内容可选 heuristic/pattern |
...
| TasteKey | 见 §7.2 | 由 payload 生成，格式 `context|dimension` 或 `scene_slug|choice_slug`；**仅存 Meta，不写入 Tags** |
| Tags | 可选 | 由 scene/choice 抽自由关键词；不包含 TasteKey 内容 |
...
| **TasteKey 存储未定** | template 增加 **TasteKey**；约定仅存 Meta，不写入 Tags，避免重复。 |
...
| **TasteKey 与 Tags 双写** | 约定 TasteKey **仅存 Meta**，Tags 仅自由关键词，不包含 TasteKey。 |
| **Kind/Strength/Scope 纯推断** | 在 §6.5.1 中改为**可执行规则**（source/apply/用户表述），减少不一致。 |

#### 6.6.3 模板与 INDEX 的同步

- **template**：已增加 Title、TasteKey、Supersedes；Status 含 local/archive（生命周期）；结构约定见 template 顶部。
- **INDEX**：表头需与 template Meta 对齐，至少包含 Id, Kind, **Title**, When to load, Status, Strength, Scope, Supersedes, CreatedAt, UpdatedAt, Source, Session, **TasteKey**（可选）, File。实现时按 memory-system 与 lingxi-memory 的 INDEX 约定更新。

#### 6.6.4 实施责任

- 更新 **memory-note-template.md**：已完成（见本仓库该文件）。
- 更新 **lingxi-memory** 与 **INDEX 读写逻辑**：写入/更新 note 与 INDEX 时按新 Meta 字段（Title、TasteKey、Supersedes）执行；治理时填写 Supersedes。
- **memory-retrieve**：检索可继续使用 Title、When to load、TasteKey（若 INDEX 或 note 含该列/字段）做关键词或语义匹配。

---

## 7. 环节品味嗅探与记忆键

以下为情境驱动的品味嗅探规则与 taste_key 约定；**非固定触发点**，以各环节内**是否出现规则所描述的情境**为准。

### 7.1 品味嗅探规则（T2 定稿）

- **定位**：不在流程中设固定「第几步弹哪道题」；而是在各环节内，当**相邻知识框架中的一种或多种待决策场景**出现时，视为可嗅探品味的时机，必要时用 questions-interaction 向用户提问并根据反馈提取品味。
...
- **taste_key**：由**情境类型 + 原则维度**生成，格式仍为 `context|dimension`，其中 context 对应情境类型、dimension 对应原则维度（见 §7.2.2）。
...
| **是否提问** | 该情境下「静默从对话抽」还是「主动用 questions-interaction 问」（如用户选了非推荐项 → 主动问理由）。 |
| **问什么** | 若提问，questions 的意图与措辞（如「请简短说明您选择该方案的主要考虑」）；选项宜与上述可命名原则/策略对齐。 |
| **如何提取** | 从用户回答或本轮对话中映射到预定义 principles/choice，填 payload（scene/principles/choice/evidence）及 taste_key（情境类型|原则维度）。 |
...
记忆键用于两件事：**① 环节选择题「已沉淀则不再问」**；**② 检索时提高命中率**（关键词或语义可命中键）。键设计直接影响记忆提取与去重效果，优先落地。

#### 7.2.1 用途

| 用途 | 说明 |
| --- | --- |
| **不再问** | 在某个 (context, dimension) 弹出选择题前，先查是否已有该键的记忆；有则静默应用已有 choice，不弹选择。 |
| **检索命中** | memory-retrieve 的关键词路径（及可选语义）可命中键，使「场景/维度」类查询能稳定召回对应记忆。 |

#### 7.2.2 键的结构与命名

- **统一键名**：`taste_key`（单字段，便于 INDEX 与 note Meta 一致）。
- **键值格式**：`<情境类型>|<原则维度>`，即 `context|dimension`，归一化后存储（小写、连字符、无空格、稳定 ID 或短英文）。
  - **context（情境类型）**：嗅探到的情境类别或场景族，如 `ux-priority`、`reuse-componentization`、`plan-non-recommended-choice`、`review-exception`、`design-taste-ui`。
  - **dimension（原则维度）**：该情境下「在什么原则/选项之间选」，如 `simplicity-vs-extension`、`experience-vs-cost`。
- **约定**：taste_key 由**情境类型 + 原则维度**生成；与环节品味嗅探规则中的情境、原则表述一致，便于「不再问」与检索。
- **非嗅探产出的记忆**（来自品味识别 auto/remember/init）：由 payload.scene 与 payload.choice 生成 `scene_slug|choice_slug`；若与某嗅探情境的 (情境类型, 原则维度) 语义等价，可共用同一 taste_key。

#### 7.2.3 存储位置

- **note Meta**：memory-note-template 中 **TasteKey** 为可选字段；**仅存于此，不重复写入 Tags**，避免冗余。写入时由 lingxi-memory 从 payload 生成。
- **INDEX**：INDEX 表增加一列 **TasteKey**（可选，与 note 一致）。便于「不再问」时只扫 INDEX 做键匹配，无需读全部 note。

#### 7.2.4 生成规则

- **环节品味嗅探**：当规则命中某情境时，由该条规则的**情境类型**与**原则维度**生成 taste_key：`taste_key = normalize(情境类型) + "|" + normalize(原则维度)`，写入 payload 或 note。
- **品味识别（auto/remember/init）**：由 payload.scene 与 payload.choice 生成稳定 slug，拼成 `scene_slug|choice_slug`；若与某嗅探规则的 (情境类型, 原则维度) 语义等价，可共用同一 taste_key。
- **归一化约定**：同一语义对应同一键（小写、连字符、无空格；中文可转拼音首字母或保留稳定英文 ID）。

#### 7.2.5 与「不再问」的配合

- 当某条品味嗅探规则的情境被命中、且拟向用户提问前：用 `taste_key = 情境类型|原则维度` 查 INDEX（或 note 的 TasteKey）。若存在且 Status=active，则**不再问**，直接按该条记忆的 One-liner/Decision 行为；若不存在或已归档，再按规则提问。
- 嗅探后写入的 note 必须带 TasteKey，以便下次同一情境类型+原则维度命中。

#### 7.2.6 与 memory-retrieve 的配合

- 检索时：**关键词路径**可对 INDEX 的 TasteKey 列（及 note 的 When to load、Title）做匹配，用户消息中的场景/维度词可与 taste_key 的子串或同义表述匹配。
- **语义路径**不变，仍对 note 正文做概念匹配；TasteKey 作为稳定信号，提高「场景-维度」类查询的召回与一致性。

#### 7.2.7 实施要点

- memory-note-template、INDEX 表头增加 **TasteKey**（可选）；lingxi-memory 在 payload→note 映射时写入 TasteKey（§6.5）。
- 各环节品味嗅探规则中的**情境类型**与**原则维度**作为 taste_key 的枚举来源，由各环节 references 维护，保证键值稳定、不重不漏。
...
| T2 | 品味嗅探规则 | **已形成方案** §7.1：各环节 references 增加品味嗅探规则（情境描述、是否提问、问什么、如何提取）；非固定触发点、以情境为准；taste_key 由情境类型+原则维度生成；示例表见 §7.1.2；执行细节类（如引用方式）仅当用户已表达为可复用规则时才抽 |
| T3 | 记忆键设计 | **已形成方案** §7.2（**优先落地**）：taste_key 结构、存储、生成、与「不再问」及 memory-retrieve 的配合 |
| T4 | 选项与原则映射表 | **已形成方案** §7.1、§7.3：在各环节嗅探规则中提前确定可命名的原则/策略，per-rule 定义；用户表述/选项映射到预定义项后填 payload |
| T5 | 选择题记忆条目的标签或类型 | **已关闭**：不单独设标签/类型；用 Source=choice 识别，TasteKey 标识决策点，Kind 表内容形态；Tags 仅自由关键词，不与 Source 重复 |
| T6 | 「跳过」与重复询问策略 | **已形成方案** §7.4：跳过则按 agent 默认方式继续、不写记忆；相同问题在本轮对话中不再重复提问 |
| T7 | payload → note 的映射规则 | **已形成方案** §6.5：Meta/When to load/One-liner/Decision 等生成规则，含 Title、TasteKey、Supersedes |
| T8 | Note 结构优化 | **已形成方案** §6.6、memory-note-template：补齐 Title/Supersedes/TasteKey，去冗余与歧义，与 INDEX 对齐；属本次重构一部分 |
