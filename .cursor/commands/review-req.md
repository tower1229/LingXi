...
本命令直接在本对话中执行 req 文档审查，不委托独立 Skill；审查维度与输出格式见下文。
...
**推荐逻辑**：无 Blockers/High → A = `/plan <taskId>` 或 `/build <taskId>`（推荐），B = 调整 req，C = 回头看 req，D = 其他。有 Blockers/High → A = 先改 req（推荐），B = `/plan` 或 `/build`，C/D = 其他。
