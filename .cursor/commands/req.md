...
**任务编号自动生成**：命令会自动扫描 `.workflow/requirements/` 目录，提取所有 `*.req.*.md` 文件的编号，取最大编号+1，从 001 开始。编号上限为 999。
...
- 文件写入成功：静默，不输出确认信息（如"已写入 001.req.xxx.md"）
- 文件写入失败：输出错误信息
...
在需求分析过程中，如发生以下情况，立即输出经验候选（HTML 注释包裹，不干扰对话）：
