...
**约定**：必须先调用 taste-recognition 产出 payload，再以 **payloads 数组**（单条时为仅含一元素的数组）与 conversation_id（及可选 generation_id）调用 lingxi-memory；lingxi-memory 仅接受 payloads 数组。主对话呈现 lingxi-memory 返回的简报或静默。
