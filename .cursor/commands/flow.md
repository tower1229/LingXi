...
- **产物落盘**：遵循既有 `ai/requirements/` 与 `ai/context/` 约定
...
- `ai/requirements/in-progress/<REQ-xxx>.md`（Requirement）
- `ai/requirements/in-progress/<REQ-xxx>.plan.md`（Plan / 执行账本）
- `ai/requirements/in-progress/<REQ-xxx>.review.md`（Review）
- `ai/requirements/INDEX.md`（SSoT）
- `ai/context/experience/*`（经验沉淀，需用户确认）
- `ai/context/session/*`（checkpoint / 会话临时）
...
> **禁止**：在用户没有明确确认“沉淀”的情况下写入 `ai/context/experience/`。
...
- 如果用户选择 **沉淀**：
  ...
  - 对每条候选，执行“即时沉淀”：
    ...
    - 按 `skill-experience-depositor.mdc` 落盘经验文件并更新 `ai/context/experience/INDEX.md`
  - 全部完成后删除暂存文件
...
- 确保 `ai/requirements/INDEX.md` 表头符合 `skill-index-manager.mdc`
- 确保 `ai/context/session/` 与 `ai/workspace/` 目录存在（若不存在则创建）
...
1. `ai/requirements/INDEX.md` 中该 REQ 的 `Current Phase` / `Status` / `Next Action`
...
> Hooks 会在对话结束时自动弹出“是否沉淀”的确认；用户必须用 `/flow 沉淀 ...` 明确确认后，才允许写入 `ai/context/experience/`。
