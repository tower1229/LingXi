...
2) **噪音过滤器**：应用 `docs/02-design/noise-filter.md` 中的判断规则，过滤明显噪音候选。被过滤的候选记录过滤理由（基于哪个模式），支持人工复核。
...
5) 暂存：写入或合并到 `.workflow/context/session/pending-compounding-candidates.json`，避免重复，保留时间戳与来源 stage。
6) 不写入经验，不触发 curator，不向用户提问；仅在必要时简短确认已接收。

**噪音过滤器判断规则**（参考 `docs/02-design/noise-filter.md`）：
