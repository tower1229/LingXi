...
3. E2E 测试执行（使用 Cursor Agent Browser 的自然语言方式）：
   ...
   **执行流程**：
   ...
   c. **执行测试场景**（示例转换方式）：
      ...
      填写密码字段为 password123
      点击"登录"按钮
      验证页面已跳转到 /dashboard，检查是否显示用户信息和用户头像元素
      截图保存当前页面状态
   ...
   e. **验证结果**：
      - 检查页面状态（通过截图）
      - 验证 URL 变更（如涉及路由跳转）
      - 验证元素存在/文本匹配（如 testcase 中定义的验证点）
      - 检查控制台错误（如有 JavaScript 错误，记录为问题）
      - 验证网络请求（如果关键流程涉及 API 调用）
...
7. 静默返回结果：
   - 不干扰主流程
   - 返回结构化审查结果
   - 格式与其他 subagents 一致（Blockers/High/Medium/Low）
