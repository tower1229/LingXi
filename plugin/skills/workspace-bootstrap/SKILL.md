...
在项目根目录执行：`node plugin/skills/workspace-bootstrap/scripts/workspace-bootstrap.mjs`。脚本读取 `references/workflow-skeleton.json`，创建缺失目录，若目标文件不存在则从 `references/` 写入模板与 INDEX 占位。
