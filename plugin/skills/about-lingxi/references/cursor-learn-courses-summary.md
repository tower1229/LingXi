...
**灵犀参考**：Session 内 hooks 注入约定、memory-retrieve 的「最小高信号」注入、子代理（如 lingxi-memory-write、explore）独立上下文，属于 AI 工作流的基础实践；避免规则/Skill 全文常驻，采用「按需加载、引用式说明」。
...
**灵犀参考**：灵犀的工作流由 **Skills**（task/plan/build/review 等，可经 `/task`、`/plan` 等或自然语言触发）驱动，本质是「带 harness 的 Agent 流程」；子代理（lingxi-memory-write、explore）承担专门任务并返回精简结果，以保护主对话上下文与成本。**选型**：子代理适合独立上下文、多步、需隔离或静默的任务；可一次性完成的任务优先用 Skill，避免不必要子代理以控制 token 消耗。
...
- **设计到代码**：可上传/插入截图/线框图，或接 Figma MCP；集成浏览器可预览与验证 UI。
- **验证**：测试、类型检查、linter、浏览器/MCP 等，让 Agent 能自验输出。

**灵犀参考**：plan、build skill 与测试设计/执行对应「规划 → 实现 → 验证」；review 时强调可验证目标（测试、类型、lint）。
...
- **Rules**（`plugin/rules/`）：每次对话加载；适合构建/测试命令、代码约定、示例引用、防护规则；宜简短、具体，用引用代替长文粘贴。
