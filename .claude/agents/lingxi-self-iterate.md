...
1. 执行提案生成（含回放评测指标）：
   - `node .claude/agents/lingxi-self-iterate/scripts/memory-improvement-proposal.mjs --window-hours 24`
2. 执行自动改进：
   - `node .claude/agents/lingxi-self-iterate/scripts/memory-improvement-apply.mjs --approve-all`
