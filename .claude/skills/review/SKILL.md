...
- **taskId**：指定则用该编号的 task；省略则执行 `node .claude/skills/task/scripts/latest-task-id.mjs` 获取最新任务编号。脚本失败则输出错误并终止。
...
1. **读取输入**
   - 扫描 `.lingxi/tasks/`。
   - 指定 taskId 用指定编号；省略时执行 `node .claude/skills/task/scripts/latest-task-id.mjs` 获取最新编号。
   - 脚本失败必须终止并输出错误。
