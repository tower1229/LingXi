---
name: about-lingxi
description: 仅在涉及灵犀架构、工作流、记忆系统、守护层、灵犀内 Command/Skill/Hook 选择与设计、或自我迭代（诊断与改进、对灵犀核心文件如 Skill/插件/Rules/refs 的评估与修改）时激活。提供核心价值、四层架构与四阶段管道心智模型、场景→加载清单→动作映射及 refs 指针；调优与方案评估时提供价值对齐与工程实践依据；自我迭代时提供标准出处与可改边界。勿在用户仅问通用编程、框架用法或业务逻辑时仅因出现「架构」「优化」等词而激活。
---
...
- **四层**：**调度层**（主 Agent 决策、四阶段管道、状态与后处理）→ **执行层**（Subagents、Megaprompt 与 `<Execution_Summary>`）→ **记忆层**（HOT_RAM、SESSION_TRACE、USER、memory/、INDEX、WAL）→ **守护层**（beforeSubmitPrompt → heartbeat-check；heartbeat-plugins 注册表，入队与消费分离）。
- **统一管道**：所有请求统一走四阶段管道——Phase 0（脚本预处理）→ Phase 1（任务预处理）→ Phase 2（任务委派）→ Phase 3（后置处理），无分支决策。
- **关键路径**：Phase 0 脚本完成 HOT_RAM 初始化与心跳 → Phase 1 读 HOT_RAM + memory-retrieve + taste-recognition → Phase 2 megaprompt-assembly + 派发 Subagent → Phase 3 写 SESSION_TRACE + 消费 `[POST-PROCESSING QUEUE]`；后处理未勾销前不得结束回复。
...
|------|-------------------------|----------|
| **快速了解灵犀** | 本页「一页纸入门」即可；需摘要时再加载 `core-values.md` + `architecture.md` | 用 1～3 句话说明四层 + 四阶段管道 + 关键路径，并给出可深入 refs |
