...
description: 显式调用。由 plan、build、review 调用，从 task 文档产出结构化 testcase 文档，保证 F→TC 映射与验证方式一致。
---
...
- **输入**：task 文档路径或正文。**输出**：与 `references/testcase-doc-template.md` 同构的 testcase 文档；写入路径 `.lingxi/tasks/<taskId>.testcase.<标题>.md`（与 task/plan 同目录），命名 `<taskId>.testcase.<标题>.md`（标题 10 字以内）。
