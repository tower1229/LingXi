...
- **任务编号**：执行 `node .claude/skills/task/scripts/next-task-id.mjs` 获取三位数编号（项目根目录）；标题最多 10 中文字符或 20 英文字符，特殊字符替换为下划线。
...
2. **任务编号和标题生成**
   - 执行 `node .claude/skills/task/scripts/next-task-id.mjs` 获取三位数编号（在项目根目录执行）。
