...
- **taskId**：指定则用该编号的 task；省略则执行 `node .claude/skills/task/scripts/latest-task-id.mjs` 获取最新任务编号。
...
- **下一步建议**：只要写入了 plan 或 testcase，必须在当轮回复末尾输出「**下一步可尝试（选一项）**」+ 四项 A/B/C/D；允许集合：执行 build skill、调整 plan、其他/跳过。

## 完整执行流程（关键步骤不省略）

1. **读取输入与定位**
   - 扫描 `.lingxi/tasks/`；指定 taskId 用指定编号，省略时执行 `node .claude/skills/task/scripts/latest-task-id.mjs`。
...
7. **质量自检与下一步建议（有产物时必须输出）**
   ...
   - 选项仅允许：执行 build skill、调整 plan、其他/跳过。
