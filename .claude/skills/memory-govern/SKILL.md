...
1. **调用脚本**：在项目根执行 `node .claude/skills/memory-govern/scripts/memory-index-sync.mjs [--root <memoryRoot>]`。脚本会：
...
3. **门控**：建议列表经 **ask-questions** 或逐条确认，用户确认后再写回 INDEX 与（必要时）note 文件；删除/合并/归档等敏感操作必须确认。
