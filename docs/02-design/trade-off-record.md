...
| **回滚线索** | 字符串数组 | 可选 | 如何回滚该决策（指针） | `["docs/02-design/data-models.md", "src/db/migration/001.sql"]` |
...
- **影响范围**：数据层、API 层
- **回滚线索**：`docs/02-design/data-models.md`, `src/db/migration/001.sql`
...
<!-- EXP-CANDIDATE
{
  ...
  "pointers": ["docs/02-design/data-models.md", "src/db/migration/001.sql"]
}
-->
```
...
|---|---------|----------|--------|----------|----------|
| 1 | 当需要选择数据存储方案时 | 选择关系型数据库而非 NoSQL | 一致性要求高于性能要求 | 使用关系型数据库，接受性能风险 | `docs/02-design/data-models.md` |
...
- **Pointers**：`docs/02-design/data-models.md`
...
- [知识沉淀机制设计](./knowledge-compounding.md) - Decision Shape 与 Judgment Capsule
- [闸门协议设计](./gate-protocol.md) - 阶段切换检查清单
