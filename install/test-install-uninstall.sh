...
if [ -f "package.json" ]; then
  if node -e 'const fs=require("fs"); const pkg=JSON.parse(fs.readFileSync("package.json","utf8")); const scripts=pkg.scripts||{}; process.exit(("lx:create-automation" in scripts || "lx:setup" in scripts || "lx:uninstall" in scripts) ? 1 : 0);'; then
    :
  else
