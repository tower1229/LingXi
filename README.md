...
- **极简入口**：
- **可伸缩工作流**：req → plan → audit → work → review → archive 的阶段化协作，让“做完了”可被记录、复盘与交接
- **质量资产化**：把过程产物沉淀为长期可读、可查、可维护的仓库资产（requirements / experience / rules / skills / hooks）
- **显式沉淀入口**：用 `/remember` 随时把经验写入系统（不依赖某个 REQ），把“刚学到的”立刻变成可复用资产
- **自迭代体系**：支持合并/取代/去噪，让经验库保持精炼；高频判断可升级为更强的表达（规则/门控/流程能力），但需要人工采纳
- **人工门控（让魂在场）**：关键阶段推进与关键写入都需要显式确认，边界清晰、责任可追溯
- **经验汲取**：指针优先、按需展开；并按场景触发加载相关经验与提醒，让上下文“少而准”
- **自迭代体系**：把阶段 Playbook（模板/清单/写入规范）下沉为 Skills，便于复用、组合与持续演进
