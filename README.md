...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/task`        | `/task <description>`<br><br>**Examples:**<br>`/task Add user login with email and phone`<br>`/task Improve homepage load, target LCP < 1s` | **Create task doc**<br><br>Auto task id (001, 002...) and title; creates:<br>`.cursor/.lingxi/tasks/001.task.<title>.md`<br><br>Core doc for the whole workflow: refined requirements, technical approach, and more.                                                              |
...
| `/build`      | `/build [taskId]`<br><br>**Examples:**<br>`/build 001`<br>`/build` (latest task)                                                         | **Run build (optional)**<br><br>Two modes:<br>- **Plan-driven**: Follow plan when present (recommended)<br>- **Req-driven**: Agent decides from task doc when no plan<br><br>**Tip:** In plan mode you can use its built-in build and skip LingXi’s `/build`.                         |
...
| `/refine-memory` | `/refine-memory` or `/refine-memory <time range>`<br><br>**Examples:**<br>`/refine-memory` (current conversation)<br>`/refine-memory 提炼今天的会话`<br>`/refine-memory 1d` | **Refine memory from conversation(s)**<br><br>Extract sedimentable content and write to memory. No args = current conversation; with args = natural-language time range (e.g. “today”, “last 2 days”, “1d”, “24h”). All payloads are sent to lingxi-memory in one batch; you get a short report. |
