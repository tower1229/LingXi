...
```text
/flow 采纳质量准则 <序号列表>   # 例如 /flow 采纳质量准则 1,3
/flow 忽略质量准则             # 全部不采纳
```
...
**人工闸门**：采纳动作需用户显式执行 `/flow 采纳质量准则`，否则不写入。
...
3. 自动执行治理动作：
   ...
   - 取代：更新 INDEX，输出变更报告
4. 生成"质量准则建议"（1-3 条），等待用户 /flow 采纳质量准则
```
...
| T5  | 扩展 `/flow` 命令：支持 `采纳质量准则 / 忽略质量准则`                      | P1     | T3   | [x]  |
| T6  | 新建 `.workflow/context/tech/quality-bar.md`（质量准则写入目标）           | P1     | T5   | [x]  |
...
| #   | 验证项                            | 预期结果                                          | 验证方式                    |
| --- | --------------------------------- | ------------------------------------------------- | --------------------------- |
| V1  | 沉淀 1 条新经验后触发成长循环     | 输出治理报告 + 质量准则建议                       | 手工执行 `/flow 沉淀 1`     |
| V2  | 新经验与旧经验 Tag 相同时自动合并 | INDEX 更新 Replaces/ReplacedBy，旧经验 deprecated | 手工构造重复经验            |
| V3  | 质量准则建议可采纳/拒绝           | 采纳后写入到 rules/skills/quality-bar；拒绝不写入 | 手工执行 `/flow 采纳质量准则 1` |
...
| 2026-01-12 | Review 通过（V1-V4 全部 Pass），进入 archive |
| 2026-01-12 | Archive 完成，需求关闭                       |
