...
| 记忆审计写入后追加 memory\_\* 行         | ✅ testcase 覆盖 | TC-006, TC-007；需 /remember 或工作流品味嗅探等写入后验证  |
...
- **High**：无
- **Medium**：无
- **Low**：审计路径固定于 `.lingxi/workspace/audit.log`，无用户可控路径；主审计对 prompt/reply/tool_output 做截断，控制体积与敏感信息；append-memory-audit 接收子代理传入的 JSON，event 等为受控取值，风险低。
