...
- **记忆提取审计（v2）**：每轮记忆提取必须追加 `memory.retrieve.performed` 或 `memory.retrieve.skipped`；若轮次缺失检索审计，系统追加 `memory.retrieve.missing`，schema 非法时记录 `memory.retrieve.invalid`。
...
| S2   | 记忆审计：子代理写 note/INDEX 后，audit 文件中出现 memory*note*\* / memory_index_updated 行          | 执行 /remember 或工作流品味嗅探等记忆写入后检查 |
...
- [ ] 存在统一审计日志文件（如 `.lingxi/workspace/audit.log`），格式为 NDJSON。
...
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | -------- |
| F1   | 主审计脚本：从 stdin 读 Hook 入参，写一条 NDJSON 到统一 audit 文件，并按事件类型返回最小合法 JSON                                                                                                     | `plugin/hooks/lingxi-audit.mjs`；复用 `_hook-utils.mjs` 的 readStdinJson；appendFileSync 到 `.lingxi/workspace/audit.log` | 各 Hook 触发后 audit 中有一行且主流程不中断                  | 必须实现 |
| F2   | hooks.json 中注册 beforeSubmitPrompt、afterAgentResponse、postToolUse、postToolUseFailure、subagentStart、subagentStop、sessionEnd、stop 调用审计脚本                                                 | `.cursor/hooks.json` 增加上述 8 个事件的 command 指向 lingxi-audit.mjs                                                             | 配置存在且路径正确（项目级 `plugin/hooks/...`）             | 必须实现 |
...
| F4   | sessionStart 约定中增加「当前会话 ID；调用 lingxi-memory 时请在 input 中传入 conversation_id（及可选 generation_id）」                                                                                | `plugin/hooks/session-init.mjs` 的 ADDITIONAL_CONTEXT 中增加上述约定；session_id 来自 sessionStart 入参                           | 新会话开始时约定中包含会话 ID 与传入要求                     | 必须实现 |
| F5   | lingxi-memory 子代理从 input 解析 conversation_id、generation_id；每次写 note/INDEX 后追加一条记忆审计行到同一 audit 文件                                                                             | `plugin/agents/lingxi-memory.md` 约定解析与写入；写入逻辑在子代理执行写入后追加 NDJSON 行                                         | 写记忆后 audit 中出现 memory*note*\* 行且含 conversation_id  | 必须实现 |
| F6   | 记忆写入审计事件类型：memory_note_created、memory_note_updated、memory_note_deleted、memory_index_updated（可选）；字段含 ts、event、conversation_id、generation_id、note_id、operation、source、file | 子代理写入后根据操作类型生成对应 event 与字段，appendFileSync 同一 audit 文件                                                      | 创建/更新/删除 note 后对应 event 与字段正确                  | 必须实现 |
| F9   | 记忆提取审计（v2）：每轮记录 `memory.retrieve.performed` 或 `memory.retrieve.skipped`；缺失时追加 `memory.retrieve.missing`；非法 schema 记录 `memory.retrieve.invalid`                               | 统一通过审计追加脚本写入；完整性检查在 afterAgentResponse 轮次执行，不阻断主流程                                                   | 任一用户输入轮次均可定位到 performed/skipped 或 missing 事件 | 必须实现 |
| F7   | INDEX 表与 note Meta 扩展：CreatedAt、UpdatedAt、Source、Session（或 CreatedInSession/UpdatedInSession）；写入/更新 note 时更新 INDEX 对应行与 note 内 Meta                                           | memory/INDEX.md 表头与行内容；memory-write skill 的 references/memory-note-template.md 与 memory-system 文档约定字段              | 新写或更新一条记忆后 INDEX 与 note 内时间/会话字段正确       | 必须实现 |
...
- Cursor Hooks：依赖 Cursor 提供的 Hook 事件与 stdio JSON 协议；项目级 hooks 路径为 `.cursor/hooks.json`，脚本路径为 `plugin/hooks/`，从项目根执行。
...
- 记忆审计由 lingxi-memory 子代理在现有「校验 → 映射 → 治理 → 门控 → 写入」流程末尾追加一次 append，不改变现有写入语义。
...
| event                  | 说明       | 事件特有字段                                                                                                                   |
| ---------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `memory_note_created`  | 记忆创建   | `note_id`: string；`operation`: string（如 `create`）；`source`: string（如 `remember`/`extract`/`choice`/`init`/`heartbeat`）；`file`: string，note 文件相对路径 |
...
`event` 取值及事件特有字段（提取，v2）：

| event                       | 说明                                     | 事件特有字段                                                                                                                                                                   |
| --------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `memory.retrieve.performed` | 记忆提取已执行                           | `query`: string；`hits`: array；`adopted`: array；`rejected`: array；`semantic_called`: boolean；`keyword_called`: boolean；`candidate_read_count`: number；`decision`: string |
| `memory.retrieve.skipped`   | 记忆提取显式跳过                         | `query`: string；`reason`: string                                                                                                                                              |
| `memory.retrieve.missing`   | 轮次完整性缺失                           | `reason`: string；可选 `expected_events`: string[]                                                                                                                             |
| `memory.retrieve.invalid`   | 事件 schema 非法（仅 retrieve 类）       | `reason`: string；可选 `invalid_event`: string                                                                                                                                 |
| `memory.write.invalid`      | 事件 schema 非法（write 类校验失败）     | `reason`: string；可选 `invalid_event`: string                                                                                                                                 |
| `memory.audit.invalid`      | 事件 schema 非法（event 缺失或不受支持） | `reason`: string；可选 `invalid_event`: string                                                                                                                                 |

当轮次已有 performed 或 skipped 时，完整性审计会追加 `memory.retrieve.keyword_path_verified`（可选），字段含 `conversation_id`、`generation_id`、`verified`: boolean（同轮是否存在 pre_tool_use 且 tool_name 为 Grep）。

心跳相关事件（会话提炼）：`heartbeat.triggered`（lingxi-session-distill 子代理在启动时写入：ts、conversation_id、candidate_ids、reason）；`heartbeat.distillation_completed`（lingxi-session-distill 子代理写入：ts、candidate_ids、sessions_processed、payloads_written、conversation_id 可选）；`heartbeat.distillation_failed`（子代理写入：ts、candidate_ids、error_preview）。

记忆审计的 `source` 取值与 INDEX/note Meta 中的 Source 约定一致（参见 memory-system）。

### 8.3 参考

- Cursor Hooks 文档：https://cursor.com/docs/agent/hooks
- 灵犀架构：`plugin/skills/about-lingxi/references/architecture.md`
- 记忆系统：`plugin/skills/about-lingxi/references/memory-system.md`
- 守护层与心跳：`plugin/skills/about-lingxi/references/architecture.md`（守护层一节）、`plugin/skills/about-lingxi/references/engineering-practices.md`
