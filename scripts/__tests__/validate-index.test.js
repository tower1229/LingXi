...
test('B2: 验证 ID 格式 - 正确格式', () => {
  const line = '| REQ-001 | Test | in-progress | req | Next |  | `.workflow/requirements/in-progress/REQ-001.md` |';
...
test('B2: 验证 ID 格式 - 数字不足3位', () => {
  const line = '| REQ-1 | Test | in-progress | req | Next |  | `.workflow/requirements/in-progress/REQ-1.md` |';
  const row = parseRow(line);
  if (row) {
...
test('B2: 验证 ID 格式 - 非数字', () => {
  const line = '| REQ-abc | Test | in-progress | req | Next |  | `.workflow/requirements/in-progress/REQ-abc.md` |';
  const row = parseRow(line);
  if (row) {
    throw new Error('期望解析失败（ID 格式不正确），但解析成功');
  }
...
test('B3: 验证 Status 值 - 有效值', () => {
  ...
  validStatuses.forEach(status => {
    const row = {
      ...
      status: status,
      currentPhase: 'req',
      links: `.workflow/requirements/in-progress/REQ-001.md`
    };
...
test('B5: 检查文件一致性 - 文件存在', () => {
  ...
  const row = {
    ...
    links: '`.workflow/requirements/completed/REQ-001.md` / `.plan.md` / `.review.md`'
  };
  const errors = validateRow(row, projectRoot);
...
test('B6: 检查目录一致性 - Status=in-progress, 文件在 in-progress/', () => {
  ...
  const row = {
    ...
    status: 'planned',
    currentPhase: 'audit',
    links: '`.workflow/requirements/in-progress/REQ-002.md` / `.plan.md`'
  };
  const errors = validateRow(row, projectRoot);
...
test('B7: 输出验证结果 - 所有验证通过', () => {
  // 使用实际项目的 INDEX.md
  const indexPath = path.join(__dirname, '../../../.workflow/requirements/INDEX.md');
