...
Your go-to toolkit for the AI era.

## How (Approach)

### 1) 心有灵犀 (Shared Understanding)
...
Lower cognitive load and a smooth experience.
...
- **Persistent memory**: Learns your judgment, taste, and responsibility in the project and applies them in every new conversation
- **Human in the loop**: Key decisions follow your lead—optional when you want, never overstepping when you don’t
- **Context management**: Optimize context so the model focuses on what matters
- **Ready to use**: Install from the Cursor plugin marketplace, then run `/init` to set up LingXi in your project
...
- **First time**: Run `/init` once in your project to create the workspace layout (`.cursor/.lingxi/`) and optional memory drafts.

---

### Quick Start

**Run `/init` first** to initialize the project (creates `.cursor/.lingxi/` skeleton and templates); then use the commands below.
...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/req`        | `/req <description>`<br><br>**Examples:**<br>`/req Add user login with email and phone`<br>`/req Improve homepage load, target LCP < 1s` | **Create task doc**<br><br>Auto task id (001, 002...) and title; creates:<br>`.cursor/.lingxi/tasks/001.req.<title>.md`<br><br>Core doc: refined requirements and technical approach.                                                                                            |
...
| `/build`      | `/build [taskId]`<br><br>**Examples:**<br>`/build 001`<br>`/build` (latest task)                                                         | **Run build (optional)**<br><br>Two modes:<br>- **Plan-driven**: Follow plan when present (recommended)<br>- **Agent-driven**: Agent decides from req when no plan<br><br>**Tip:** In plan mode you can use its built-in build and skip LingXi’s `/build`.                       |
...
| Command     | Usage                                                                                                                            | Description                                                                                                                                                                                                                                                                                |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/remember` | `/remember <description>`<br><br>**Examples:**<br>`/remember Capture the lesson from that bug`<br>`/remember Always use X for Y` | **Write to memory (any time)**<br><br>No task id needed. Write judgments, tradeoffs, runbooks, or checks to `memory/notes/` for later retrieval.<br><br>**Use when:**<br>- Stating a principle or decision<br>- Extracting from recent conversation<br>- Giving keywords to locate content |
| `/init`     | `/init`                                                                                                                          | **Initialize project (first use)**<br><br>Guided collection of project info (stack, patterns, rules), then optional write of initial memory to `memory/notes/`. Run when first using LingXi in a project.                                                                                  |

#### Sharing across projects (share dir + git submodule)

LingXi uses a fixed share directory for team knowledge that can be reused across projects:
...
- [Architecture](.cursor/skills/about-lingxi/references/architecture.md)
