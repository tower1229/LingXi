...
5. **写入**：按 payload.apply 决定路径——**apply === "team"** 时写入 `.lingxi/memory/share/MEM-<id>.md`，INDEX 的 File 列为 `memory/share/MEM-<id>.md`；否则写入 `.lingxi/memory/project/MEM-<id>.md`，File 列为 `memory/project/MEM-<id>.md`。读一次 INDEX 与现有 note 得最大 MEM-id，本批内顺序分配 id，本批全部处理完后一次性写回 INDEX；每条写入后向 `MEMORY_JOURNAL.jsonl` 追加审计。
